...
@section('title', 'Saved Applications Tracking')

@section('main-content')
    
    <header class="py-6 border-b border-gray-200 mb-6">
        <h1 class="text-3xl font-bold text-gig-purple">Saved Applications Tracking Page</h1>
        <p class="text-gray-600 mt-2">You can track all the applications saved so far here. All the best in your search!</p>
    </header>
    ...
    <div class="bg-white rounded-xl shadow-md overflow-hidden border border-gray-100/70">
        <table class="min-w-full divide-y divide-gray-200">
            ...
            <tbody class="divide-y divide-gray-200">
                ...
                {{-- Placeholder Loop for Saved Jobs --}}
                @for ($i = 0; $i < 8; $i++)
                    @php 
                        $status = ($i % 3 == 0 || $i % 3 == 3) ? 'Removed' : (($i % 3 == 1) ? 'Available' : 'Pending'); 
                        $statusClass = ($status == 'Removed') ? 'text-red-500' : (($status == 'Available') ? 'text-green-500' : 'text-orange-500');
                    @endphp
                    <tr>
                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">
                            {{ $i % 2 == 0 ? 'Social Media Assistant' : 'Tutor Math' }}
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">Scholar Hub</td>
                        <td class="px-6 py-4 whitespace-nowrap">
                            {{-- Status styling --}}
                            <span class="inline-flex text-xs leading-5 font-semibold {{ $statusClass }}">
                                {{ $status }}
                            </span>
                        </td>
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium space-x-2">
                            {{-- View Details Link --}}
                            <a href="{{ route('gigs.show', 1) }}" class="text-gig-purple hover:text-purple-900">View</a>
                            {{-- Delete/Remove Link --}}
